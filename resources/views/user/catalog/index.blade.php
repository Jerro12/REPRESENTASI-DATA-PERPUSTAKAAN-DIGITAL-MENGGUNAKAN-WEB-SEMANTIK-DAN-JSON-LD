            ...
            <div class="flex-1">
                ...
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                        @foreach ($books as $b)
                            <a href="{{ route('katalog.show', $b->id) }}" class="group block h-full">
                                <div class="bg-card border border-border rounded-3xl overflow-hidden hover:shadow-2xl transition-all hover:-translate-y-2 flex flex-col h-full shadow-sm">
                                    <div class="aspect-[4/3] relative overflow-hidden bg-secondary">
                                        @if($b->cover)
                                            <img src="{{ asset('storage/' . $b->cover) }}" alt="{{ $b->judul }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                        @else
                                        ...
                                        <div class="absolute top-4 left-4">
                                            <span class="badge-info backdrop-blur-md px-3 py-1 text-[10px] font-black uppercase tracking-widest shadow-lg">{{ $b->category->nama ?? 'Buku' }}</span>
                                        </div>
                                    </div>
                                    <div class="p-6 flex-1 flex flex-col">
                                        <h3 class="font-bold text-foreground text-lg group-hover:text-primary transition-colors line-clamp-1 truncate">{{ $b->judul }}</h3>
                                        <p class="text-sm text-muted-foreground mt-1 mb-4 flex-1">{{ $b->penulis }}</p>
